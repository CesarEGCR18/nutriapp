...
export default function Index({auth, mealtimes}) {
    ...
    const openFormModal = (titulo, mealtime = {}) => {
        setModalForm(true);
        setTitulo(titulo);
    ...
    const submitData = (e) => {
        ...
        if (titulo === "Crear") {
            ...
            patch(route('mealtimes.update', data), {
                onSuccess: () => {
                    console.log("SUCCESS");
                    closeFormModal();
                },
    ...
    const onDelete = (mealtime) => {
        destroy(route('mealtimes.destroy', mealtime), {
            onSuccess: () => {
                console.log("SUCCESS");
            },
            onError: () => {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Tiempos alimienticios</h2>}
        >
            <Head title="Tiempos alimienticios"/>

            <div className='grid v-screen place-items-center mt-4'>
                <div className="mt-3 mb-3 flex justify-end">
                    <PrimaryButton className='w-auto m-1 focus:hover:bg-yellow-600 bg-yellow-600 hover:bg-yellow-700 dark:focus:hover:bg-yellow-500 dark:hover:!bg-yellow-500 dark:hover:text-white' title="Crear Tiempo" onClick={() => openFormModal('Crear')}><FontAwesomeIcon className="text-lg mr-2" icon={['far', 'square-plus']} />Crear Tiempo</PrimaryButton>
                </div>
            </div>

            {mealtimes && mealtimes.length > 0 ? (
                <div className='mx-auto'>
                    <table className='table-fixed bg-gray-800 rounded-xl overflow-hidden w-auto mx-auto my-4'>
                        ...
                        {mealtimes.map((mealtime, i) => (
                            <tr key={mealtime.id}>
                                <td className='px-4 py-3'>{mealtime.nombre}</td>
                                <td className='w-full px-4 py-3 inline-flex justify-end'>
                                    <PrimaryButton className='w-auto m-1 !bg-green-500 focus:hover:bg-green-600 hover:!bg-green-600 dark:focus:hover:bg-green-500 dark:hover:!bg-green-500 dark:hover:text-white' title="Editar" onClick={() => openFormModal('Editar', mealtime)}><FontAwesomeIcon className="text-lg" icon={['far', 'pen-to-square']} /></PrimaryButton>
                                    <PrimaryButton className='w-auto m-1 !bg-red-500 focus:hover:bg-red-600 hover:!bg-red-600 dark:focus:hover:bg-red-500 dark:hover:!bg-red-500 dark:hover:text-white' title="Eliminar" onClick={() => onDelete(mealtime.id)} disabled={mealtime.detail_plans.length > 0}><FontAwesomeIcon className="text-lg" icon={['far', 'trash-can']} /></PrimaryButton>
                                </td>
                            </tr>
                        ))}
                        </tbody>
